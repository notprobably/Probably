...
make_assertion("key",     "'%s' to have the key '%s'",                      function(a, b) return a[b] ~= nil end)

local function compare(a, b)
  local type1, type2 = type(a), type(b)
  assertions['assert_equal'](type1, type2)
  if type1 ~= 'table' and type2 ~= 'table' then assertions['assert_equal'](a, b) end
  if a == b then return true end
  for k, v in pairs(a) do
    assertions['assert_key'](b, k)
    if not compare(v, b[k]) then return false end
  end
  for k, _ in pairs(b) do
    assertions['assert_key'](a, k)
  end
  return true
end

make_assertion("same",         "'%s' to be the same as '%s'",            function(a, b) return compare(a, b) end)
